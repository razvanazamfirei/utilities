*! version 6.4 11JAN2022 DIME Analytics dimeanalytics@worldbank.org
	...
	qui {
		...
		syntax ,  Versionnumber(string) [noclear maxvar(numlist) matsize(numlist) Quietly veryquietly Custom(string) permanently ]
		...
		* Based on versions listed here: https://www.stata.com/support/faqs/resources/history-of-stata/
		local stata_versions "15 15.0 15.1 16 16.0 16.1 17.0 17 17.1"
		...
		*********************************/
		...
		if  "`veryquietly'" == "" {
			...
			noi di ""
			noi di "{phang}{err:IMPORTANT:} The most important setting of this command – the version – cannot be set inside the command due to technical reasons. {stata \`r(version)': Click here to complete set-up} {p_end}"
		}
